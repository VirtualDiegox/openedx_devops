...
- LMS at https://courses.unedx.edu
- CMS at https://studio.courses.unedx.edu
- CDN at https://cdn.courses.unedx.edu linked to a public read-only S3 bucket named courses-unedx-global-storage
- Grafana at https://grafana.courses.unedx.edu/login
  - user: admin
  - pwd: prom-operator
- public ssh access via a t2.micro Ubuntu 20.04 LTS bastion EC2 instance at bastion.courses.unedx.edu
- daily data backups archived into a private S3 bucket named prod-unedx-global-mongodb-backup
...
- LMS at https://dev.courses.unedx.edu
- CMS at https://studio.dev.courses-unedx.edu
- CDN at https://cdn.dev.courses.unedx.edu linked to an S3 bucket named dev-unedx-global-storage
- daily data backups archived into an S3 bucket named dev-unedx-global-mongodb-backup
...
.. code-block:: shell
  ...
  ssh bastion.courses.unedx.edu -i path/to/unedx-global-live-bastion.pem

  mysql -h mysql.courses.unedx.edu -u root -p

  mongo --port 27017 --host mongo.master.courses.unedx.edu -u root -p
  mongo --port 27017 --host mongo.reader.courses.unedx.edu -u root -p

  redis-cli -h redis.primary.courses.unedx.edu -p 6379
...
Use `this automated Github Actions workflow <https://github.com/VirtualDiegox/openedx_devops/actions/workflows/tutor_build_image.yml>`_ to build a customized Open edX Docker container based on the latest stable version of Open edX (current nutmeg.2) and
...
Use `this automated Github Actions workflow <https://github.com/VirtualDiegox/openedx_devops/actions/workflows/tutor_deploy_prod.yml>`_ to deploy your customized Docker container to a Kubernetes Cluster.
...
- **MySQL**. uses `AWS RDS <https://aws.amazon.com/rds/>`_ for all MySQL data, accessible inside the vpc as mysql.courses.unedx.edu:3306. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/environments/prod/rds/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **MongoDB**. uses `AWS DocumentDB <https://aws.amazon.com/documentdb/>`_ for all MongoDB data, accessible insid the vpc as mongodb.master.courses.unedx.edu:27017 and mongodb.reader.courses.unedx.edu. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_, and other common configuration settings `are located here <terraform/modules/documentdb>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
- **Redis**. uses `AWS ElastiCache <https://aws.amazon.com/elasticache/>`_ for all Django application caches, accessible inside the vpc as cache.courses.unedx.edu. Instance size settings are located in the `environment configuration file <terraform/environments/prod/env.hcl>`_. This is necessary in order to make the Open edX application layer completely ephemeral. Most importantly, user's login session tokens are persisted in Redis and so these need to be accessible to all app containers from a single Redis cache. Common configuration settings `are located here <terraform/environments/prod/redis/terragrunt.hcl>`_. Passwords are stored in `Kubernetes Secrets <https://kubernetes.io/docs/concepts/configuration/secret/>`_ accessible from the EKS cluster.
